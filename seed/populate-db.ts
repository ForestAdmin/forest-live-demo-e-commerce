...
import { populateWarehouses } from "./warehouses";


import { pool } from './database';
import {populateStorageUnits} from "./storage_units";
import {populateTransactions} from "./transactions";
...
(async () => {
  ...
  process.stdout.write('.');
  const warehouseIds = await populateWarehouses();
  process.stdout.write('.');
  const storageUnitsIds = await populateStorageUnits(warehouseIds, productIds);
  process.stdout.write('.');
  const transactionIds = await populateTransactions(orderIds, customerIssueIds);
  process.stdout.write('.');
